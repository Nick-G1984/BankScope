...
export default function SignUpPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // If email confirmation is disabled in Supabase, the user is immediately active
      if (data.session) {
        router.push(next)
        return
      }
        ...
        <div className="w-full max-w-md">
          <div className="card p-8">
            ...
            <div className="mt-6 text-center text-sm text-gray-500">
              ...
              <Link href={`/auth/sign-in${next !== '/workspace' ? `?next=${next}` : ''}`} className="text-brand-600 font-medium hover:underline">
                Sign in →
              </Link>
            </div>
          </div>
